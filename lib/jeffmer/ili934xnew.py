...
#
# 2023-0907 PP fixed importError glcdfont
#
import time
import ustruct
import framebuf
...
class ILI9341:
    ...
    def blit(self, bitbuff, x, y, w, h):
        ...
        self._writeblock(x, y, x + w - 1, y + h - 1, None)
        written = 0
        # PP modification to fix index out of range for 'c'!
        #print(f"length _buf: {len(self._buf)}")
        ...
        for iy in range(h):
            for ix in range(w):
                ...
                #DEBUG: print(f"c:{c}")
                if (c == 0 | c == 1) :  # assume text
                    self._buf[index*2] = self._colormap[c*2]
                    self._buf[index*2+1] = self._colormap[c*2+1]
                else:   # graphics
                # 2023-0907 PP do not ask why, but c not using as index
                # fixes IndexError. c = 0xff, colormap is B/W (too small)
                    self._buf[index*2] = c   
    ...
    # =============================================================
    # 2023-0901 PP added various methods like draw_image()
    # =============================================================    
    @property
    def rotation(self):
    ...
    @rotation.setter
    def rotation(self, r):
        assert 0 <= r < 8, f"Invalid value for rotation: {r}"
        self._rotation = r    # set rotation argument
        if self._rotation == 0:                  # 0 deg
            self._write(_MADCTL, b"\x48")
            self.width = self._init_height
            self.height = self._init_width
    ...
    # check for off_grid - from rdagger/Rototron
    ...
    # draw_image - partial from rdagger/Rototron, and issue Blit bitbuffer #11
    def draw_image(self, path, x=0, y=0, w=320, h=240):
        """ Draw image from flash.
            ...
            """
        # check if (x,y) extend past display dimensions
        x2 = x + w - 1
        y2 = y + h - 1
        #DEBUG: print(f"x={x}, y={y}, w:{w}, h={h}, x2={x2}, y2={y2}")
...
if __name__ == "__main__":
    # TODO: replace import with basic code
    import test_ili934xnew
